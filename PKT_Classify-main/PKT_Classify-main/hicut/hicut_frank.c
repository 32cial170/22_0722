...
int get_cut_bit_number(struct hicut_leaf * input_node, int dimension, int back_bit_len, int rule_num){
	...
	if(dimension == 1){
		...
		while(1){
			...
			if(spmf >= rule_child + pow(2, bit_cut_count+1)){
				bit_cut_count ++ ;
				rule_child = 0;
				temp = input_node;
			}
	...
	else if(dimension == 4)
	{
		...
		while(1){
			while(temp != NULL){
				unsigned  int down = ((temp -> d_port_s << back_bit_len) >> back_bit_len) >> (32 - (bit_cut_count + 1) - back_bit_len);
				unsigned  int top = ((temp -> d_port_e << back_bit_len) >> back_bit_len) >> (32 - (bit_cut_count + 1)  - back_bit_len);
				if(top < down)
					top = down;
				for(unsigned  int i = down; i <= top; ++i){
					++rule_child;
...
void build_hicut(struct hicut_node * cut_node){
	...
	if(cut_bit_number <= 0 || max_dimension_num <= 5)
		return;
	...
	cut_node -> down_pointer = (struct hicut_node **)malloc(sizeof(struct hicut_node *) * pownum);
	if(cut_node -> down_pointer == NULL){
		printf(" out of memory\n");
	}
	pointer_num += pownum;
	//printf("pownum = %d\n", pownum);
	...
	while (temp != NULL){
		unsigned int temp_ip_down, temp_ip_up;
		if(cut_node -> dimension == 1 || cut_node -> dimension == 2){
			...
			if(cut_node -> dimension == 3){
				...
				/*
				if(temp -> d_port_s == 1700, temp -> d_port_e == 1750){
					printf("dimension : %d, bit : %d, back : %d\n", cut_node -> dimension, cut_bit_number, cut_node -> back_cut_len[cut_node->dimension - 1]);
					printf("ip down : %u, ip up : %u\n", temp_ip_down, temp_ip_up);
				}
				*/
			}
		}
		...
		for(unsigned int i = temp_ip_down; i <= temp_ip_up; ++ i){
			//printf("b_3\n");
			cut_node -> down_pointer[i] = move_leaf_node(cut_node -> down_pointer[i], temp);
			cut_node -> down_pointer[i]->rule_num += 1;
			//printf("b_4\n");
			for(int k=0; k<5; ++k){
				cut_node -> down_pointer[i]->back_cut_len[k] = cut_node -> back_cut_len[k];
			}
			cut_node -> down_pointer[i]->back_cut_len[cut_node -> dimension -1] += cut_bit_number;
			/*
			if(temp -> d_port_s == 1700, temp -> d_port_e == 1750){
				printf("dimension : %d, bit : %d, back : %d\n", cut_node -> dimension, cut_bit_number, cut_node -> back_cut_len[cut_node->dimension - 1]);
				printf("ip down : %u, ip up : %u\n", temp_ip_down, temp_ip_up);
	...
	if(cut_node -> dimension == 1 || cut_node -> dimension == 2){
		...
		if(cut_node -> dimension == 1){
			...
			while(temp != NULL){
				...
				if(temp -> s_len >= (cut_node -> bit_len + back_bit_len)){
					...
					unsigned int temp_ip = ((temp -> s_ip << back_bit_len) >> back_bit_len) >> (32 - cut_node -> bit_len - back_bit_len);
					//printf("part4.1.1, temp_ip : %u, s_ip : %u\n", temp_ip, temp -> s_ip);
					cut_node -> down_pointer[temp_ip] = move_leaf_node(cut_node -> down_pointer[temp_ip], temp);
					cut_node -> down_pointer[temp_ip]->rule_num += 1;
					for(int i=0; i<5; ++i){
						cut_node -> down_pointer[temp_ip]->back_cut_len[i] = cut_node -> back_cut_len[i];
					}
					cut_node -> down_pointer[temp_ip]->back_cut_len[cut_node -> dimension -1] += cut_bit_number ;
					//printf("part4.2\n");
				}
				else
				{
					...
					unsigned int temp_ip_down = ((temp -> s_ip << back_bit_len) >> back_bit_len) >> (32-temp->s_len) << min((cut_node -> bit_len + back_bit_len  - temp->s_len), cut_node -> bit_len);
					unsigned int temp_ip_up = (((temp -> s_ip << back_bit_len) >> back_bit_len) >> (32-temp->s_len));
					for(int i=0; i < min((cut_node -> bit_len + back_bit_len - temp->s_len), cut_node -> bit_len); ++i){
						temp_ip_up = temp_ip_up << 1;
						temp_ip_up = temp_ip_up + 1;
					}
					//printf("dimension 1, temp_ip_down : %u, temp_ip_up : %u, back_bit_len : %d, temp->bitlen : %d, s_ip : %u, d_ip : %u\n",temp_ip_down, temp_ip_up, back_bit_len, cut_node -> bit_len, temp -> s_ip, temp -> d_ip);
					//printf("down : %u, top : %u, rule count : %d\n", temp_ip_down, temp_ip_up, rule_count);
					for(unsigned int i = temp_ip_down; i <= temp_ip_up; ++ i){
						cut_node -> down_pointer[i] = move_leaf_node(cut_node -> down_pointer[i], temp);
						cut_node -> down_pointer[i]->rule_num += 1;
						for(int j=0; j<5; ++j){
							cut_node -> down_pointer[i]->back_cut_len[j] = cut_node -> back_cut_len[j];
						}
						cut_node -> down_pointer[i]->back_cut_len[cut_node -> dimension -1] += cut_bit_number ;
					}
					//printf("part5.2\n");
			...
			while(temp != NULL){
				if(temp -> d_len >= (cut_node -> bit_len + back_bit_len)){
					//printf("part 7.1\n");
					unsigned int temp_ip = ((temp -> d_ip << back_bit_len) >> back_bit_len) >> (32 - cut_node -> bit_len - back_bit_len);
					cut_node -> down_pointer[temp_ip] = move_leaf_node(cut_node -> down_pointer[temp_ip], temp);
					cut_node -> down_pointer[temp_ip]->rule_num += 1;
					for(int i=0; i<5; ++i){
						cut_node -> down_pointer[temp_ip]->back_cut_len[i] = cut_node -> back_cut_len[i];
				...
				{
					//printf("part 8.1\n");
					unsigned int temp_ip_down = ((temp -> d_ip << back_bit_len) >> back_bit_len) >> (32-temp->d_len) << min((cut_node -> bit_len + back_bit_len  - temp->d_len), cut_node -> bit_len);
					unsigned int temp_ip_up = ((temp -> d_ip << back_bit_len) >> back_bit_len) >> (32-temp->d_len);
					for(int i=0; i < min((cut_node -> bit_len + back_bit_len - temp->d_len), cut_node -> bit_len); ++i){
						temp_ip_up = temp_ip_up << 1;
						temp_ip_up = temp_ip_up + 1;
					}
					//printf("dimension 2, temp_ip_down : %u, temp_ip_up : %u, back_bit_len : %d, temp->bitlen : %d, s_ip : %u, d_ip : %u\n",temp_ip_down, temp_ip_up, back_bit_len, cut_node -> bit_len, temp -> s_ip, temp -> d_ip);
					//printf("d_len : %d, d_ip : %u\n", temp->d_len, temp -> d_ip);
					//printf("part 8.1.1\n");
					//printf("down : %u, top : %u\n", temp_ip_down, temp_ip_up);
					for(unsigned int i = temp_ip_down; i <= temp_ip_up; ++ i){
						cut_node -> down_pointer[i] = move_leaf_node(cut_node -> down_pointer[i], temp);
		...
		if(cut_node -> dimension == 3){
			struct hicut_leaf * temp = cut_node -> hicut_leafnode;
			while(temp != NULL){
				unsigned  int down = ((temp -> s_port_s << back_bit_len) >> back_bit_len) >> (32 - cut_node -> bit_len - back_bit_len);
				unsigned  int top = ((temp -> s_port_e << back_bit_len) >> back_bit_len) >> (32 - cut_node -> bit_len  - back_bit_len);
				for(unsigned  int i = down; i <= top; ++i){
					cut_node -> down_pointer[i] = move_leaf_node(cut_node -> down_pointer[i], temp);
					cut_node -> down_pointer[i]->rule_num += 1;
					for(int j=0; j<5; ++j){
			...
			struct hicut_leaf * temp = cut_node -> hicut_leafnode;
			while(temp != NULL){
				unsigned  int down = ((temp -> d_port_s << back_bit_len) >> back_bit_len) >> (32 - cut_node -> bit_len - back_bit_len);
				unsigned  int top = ((temp -> d_port_e << back_bit_len) >> back_bit_len) >> (32 - cut_node -> bit_len - back_bit_len);
				for(unsigned  int i = down; i <= top; ++i){
					cut_node -> down_pointer[i] = move_leaf_node(cut_node -> down_pointer[i], temp);
					cut_node -> down_pointer[i]->rule_num += 1;
					for(int j=0; j<5; ++j){
		...
		struct hicut_leaf * temp = cut_node -> hicut_leafnode;
		while(temp != NULL){
			unsigned int temp_ip = temp -> type;
			//printf("a, temp_ip : %u\n", temp_ip);
			//if(temp == NULL)
				//printf("temp is null\n");
			//if(cut_node -> down_pointer[temp_ip] == NULL)
				//printf("cut_node -> down_pointer[temp_ip] is null\n");
			cut_node -> down_pointer[temp_ip] = move_leaf_node(cut_node -> down_pointer[temp_ip], temp);
			cut_node -> down_pointer[temp_ip]->rule_num += 1;
			//printf("b\n");
			for(int i=0; i<5; ++i){
...
void hicut_search(unsigned int input_ip_s, unsigned int input_ip_d, unsigned  int input_s_port_s, unsigned  int input_s_port_e, unsigned  int input_d_port_s, unsigned  int input_d_port_e, unsigned int input_type){
	...
	*/
	...
	/*
	if(temp -> bit_len > 0){
		printf("temp bit error : %d\n", temp -> bit_len);
	}
	if(temp -> dimension > 0){
		printf("temp dimension error : %d\n", temp -> dimension);
	}
	*/
